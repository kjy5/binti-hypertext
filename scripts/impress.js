 ...
 * impress.scripts
 *
 * impress.scripts is a presentation tool based on the power of CSS3 transforms and transitions
...
// Let me show you the cogs that make impress.scripts run...
( function( document, window ) {
    ...
    // This is where the root elements of all impress.scripts instances will be kept.
    ...
    // It's the core `impress` function that returns the impress.scripts API
    // for a presentation based on the element with given id ("impress"
    // by default).
    var impress = window.impress = function( rootId ) {

        // If impress.scripts is not supported by the browser return a dummy API
        ...
        // STEP EVENTS
        //
        // There are currently two step events triggered by impress.scripts
        ...
        var goto = function( el, duration, reason, origEvent ) {
            ...
            // go back to version 0.5.2 of impress.scripts:
        ...
        // Store and return API for given impress.scripts root element
        return ( roots[ "impress-root-" + rootId ] = {
            init: init,
            goto: goto,
            next: next,
    ...
    // Library factories are defined in src/lib/*.scripts, and register themselves by calling
        ...
        for ( var libname in libraryFactories ) {
            if ( libraryFactories.hasOwnProperty( libname ) ) {
                if ( lib[ libname ] !== undefined ) {
                    throw "impress.scripts ERROR: Two libraries both tried to use libname: " +  libname;
                }
...
//
// I've learnt a lot when building impress.scripts and I hope this code and comments
...
( function( document, window ) {
    ...
    // Note: These could also be recorded by the code in impress.scripts core as these values
    ...
    var resetStartingState = function( rootId ) {
        ...
        // Note: We remove the ones set by impress.scripts core. Otoh, we didn't preserve any original
        ...
        if ( rootsCount === 0 ) {
            ...
            // We need to remove or reset the meta element inserted by impress.scripts
            var metas = document.head.querySelectorAll( "meta" );
            for ( i = 0; i < metas.length; i++ ) {
                var m = metas[ i ];
                if ( m.name === "viewport" ) {
...
( function( document ) {
    ...
    // While waiting for a shared library of utilities, copying these 2 from main impress.scripts
    var css = function( el, props ) {
        var key, pkey;
        for ( key in props ) {
            if ( props.hasOwnProperty( key ) ) {
    ...
    // Wait for impress.scripts to be initialized
    document.addEventListener( "impress:init", function( event ) {
        api = event.detail.api;
        util = api.lib.util;
        root = event.target;
...
( function( document, window ) {
    ...
    var preInit = function() {
        if ( window.markdown ) {
            ...
            // Unlike the other extras, Markdown.scripts doesn't by default do anything in
    ...
    // Note: Markdown.scripts should run early/first, because it creates new div elements.
 ...
 *
 * TODO: Currently it is not possible to use TAB to navigate between form elements. Impress.scripts, and
...
/**
 * Adds a presenter console to impress.scripts
 *
...
// This file contains so much HTML, that we will just respectfully disagree about scripts
...
( function( document, window ) {
    ...
    // TODO: Move this to src/lib/util.scripts
    var triggerEvent = function( el, eventName, detail ) {
        var event = document.createEvent( 'CustomEvent' );
        event.initCustomEvent( eventName, true, true, detail );
        el.dispatchEvent( event );
    ...
    var impressConsole = window.impressConsole = function( rootId ) {
        ...
        // New API for impress.scripts plugins is based on using events
 ...
 *
 * As you can see this part is separate from the impress.scripts core code.
 * It's because these navigation actions only need what impress.scripts provides with
 * its simple API.
 *
 * This plugin is what we call an _init plugin_. It's a simple kind of
 * impress.scripts plugin. When loaded, it starts listening to the `impress:init`
 ...
 * core impress.scripts functionality is the `impress:init` method, as well as using
...
( function( document ) {
    ...
    document.addEventListener( "impress:init", function( event ) {
        ...
        //   in the impress.scripts code where window is scrolled to 0,0 on every presentation
        //   step, because sometimes browser scrolls viewport because of the focused element?
        //   Well, the [tab] key by default navigates around focusable elements, so clicking
        //   it very often caused scrolling to focused element and breaking impress.scripts
...
( function( document ) {
    ...
    var addNavigationControls = function( event ) {
        ...
        gc.addEventListener( root, 'impress:steprefresh', function( event ) {
            ...
            // As impress.scripts core now allows to dynamically edit the steps, including adding,
 ...
 * as much as is the case with the absolute coordinates supported by impress.scripts
 * core.
 *
 * Example:
 *
 ...
 *
 * This plugin is a *pre-init plugin*. It is called synchronously from impress.scripts
            ...
            if ( ref ) {
                ...
                if ( el.compareDocumentPosition( ref ) & Node.DOCUMENT_POSITION_PRECEDING ) {
                    ...
                    window.console.error(
                        "impress.scripts rel plugin: Step \"" + data.relTo + "\" is not defined " +
                ...
                // Step not found
                window.console.warn(
                    "impress.scripts rel plugin: \"" + data.relTo + "\" is not a valid step in this " +
                    "impress.scripts presentation. Please check your markup. Ignoring data-rel-to " +
...
( function( document, window ) {
    ...
    // Copied from core impress.scripts. Good candidate for moving to src/lib/util.scripts.
    var triggerEvent = function( el, eventName, detail ) {
        var event = document.createEvent( "CustomEvent" );
        event.initCustomEvent( eventName, true, true, detail );
        el.dispatchEvent( event );
